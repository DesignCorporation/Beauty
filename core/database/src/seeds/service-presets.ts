import { Prisma, PrismaClient, SalonType } from '@prisma/client';
...
export const SALON_CATEGORY_PRESETS: Record<SalonType, CategoryPreset[]> = {
  HAIR: [
    {
      name: 'Стрижки',
      icon: 'scissors',
...
export const SALON_SERVICE_PRESETS: Record<SalonType, ServicePreset[]> = {
  HAIR: [
    {
      category: 'Стрижки',
...
const buildServiceKey = (category: string, subcategory: string, key: string) =>
...
export async function seedDefaultCategories(
  ...
  const presets = SALON_CATEGORY_PRESETS[salonType] ?? [];
...
export async function seedDefaultServices(
  ...
  const presets = SALON_SERVICE_PRESETS[salonType] ?? [];
