...
export async function seedDefaultPermissions(prisma: PrismaClientLike): Promise<void> {
...
export async function seedDefaultRoles(prisma: PrismaClientLike): Promise<void> {
  await seedDefaultPermissions(prisma)

  for (const roleName of Object.values(UserRole)) {
    const typedRole = roleName as UserRole
