...
export default function AuthHero({
  ...
  backgroundVideoUrl,
  backgroundGallery,
  slideshowIntervalMs = 9000
}: AuthHeroProps) {
  const gallery = useMemo(() => backgroundGallery?.filter(Boolean) ?? [], [backgroundGallery])
    ...
    <div className="relative hidden min-h-screen overflow-hidden bg-gradient-to-br from-primary/10 via-primary/20 to-primary/30 lg:flex">
        ...
        {hasGallery && (
          <div className="auth-hero-gallery">
            {gallery.map((imageUrl, index) => (
              <div
                key={imageUrl}
                className={clsx('auth-hero-gallery-layer', index === activeImageIndex && 'is-active')}
                style={{ backgroundImage: `url(${imageUrl})` }}
              />
            ))}
          </div>
        )}
        <div className="absolute inset-0 bg-gradient-to-br from-primary/60 via-primary/25 to-secondary/40" />
      ...
      <div className="relative z-10 flex w-full flex-col justify-between px-12 py-14 text-white">
        <div>
          ...
          <h2 className="auth-hero-title mt-4 text-[3rem] text-transparent sm:text-[4rem] lg:text-[5rem] bg-gradient-to-r from-amber-100 via-yellow-300 to-amber-200 bg-clip-text drop-shadow-[0_5px_25px_rgba(250,204,21,0.45)]">
            {title}
          </h2>
